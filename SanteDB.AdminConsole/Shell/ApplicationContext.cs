...
namespace SanteDB.AdminConsole.Shell
{
    ...
    public class ApplicationContext : IServiceProvider
    {
        ...
        {
            this.ApplicationName = configuration.AppId ?? "org.santedb.oizac";
            ...
            while (!retVal)
            {
                ...
                try
                {
                    var principal = (authenticationProvider as OAuthIdentityProvider)?.Authenticate(
                        new ClaimsPrincipal(new ClaimsIdentity(new GenericIdentity(this.m_configuration.User, "RQO"), new Claim[] { new Claim(SanteDBClaimTypes.SanteDBScopeClaim, context.Description.Endpoint[0].Address) }, "OAUTH2", ClaimsIdentity.DefaultNameClaimType, ClaimsIdentity.DefaultRoleClaimType)), this.m_configuration.Password) ??
